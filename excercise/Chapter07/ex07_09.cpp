// Q. 여기에서는 배열과 구조체를 다루는 함수를 작성하는 것을 연습한다. 다음에 주어지는 것은 프로그램의 골격이다. 주석문으로 서술된 함수들을 작성하여 프로그램을 완성하라.
// [#include <iostream>]
// [using namespace std;]
//
// [const int SLEN = 30;]
...
int getinfo(student pa[], int n)
{
	...
	cout << "배열이 모두 차거나 학생 이름에 공백이 입력될 경우 입력이 종료됩니다.\n";

	for (int i = 0; i < n; ++i)
	{
		cout << "#" << i + 1 << "번 학생의 이름을 입력하십시오: ";
		...
		cout << "#" << i + 1 << "번 학생의 취미를 입력하십시오: ";
		cin.getline(pa[i].hobby, SLEN);

		cout << "#" << i + 1 << "번 학생의 ooplevel을 입력하십시오: ";
		if (!(cin >> pa[i].ooplevel).get())
		{
			cout << "숫자를 입력하지 않았습니다.\n";
