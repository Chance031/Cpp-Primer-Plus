...
// [};]
//
// (두 개의 형태가 어떻게 쓰이는지 비교해 보기 위해서 string 객체와 문자 배열 둘 다 사용한다.)
// 정의되지 않은 메서드들을 위한 코드를 제공함으로써 구현을 완성하는 프로그램을 작성하라. 그 클래스를 사용하는 프로그램은 세 가지 가능한 생성자 호출(매개변수 없음, 하나의 매개변수, 두 개의 매개변수)과 두 개의 디스플레이 메서드를 사용해야 한다. 다음은 생성자들과 메서드들을 사용하는 예이다.
